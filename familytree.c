...
void* getdata(int* noc)
{
    ...
    while(1)
    {
        printf("\nEnter No. of children for %s:\t",data);
        if(scanf("%d",noc)!=1)
        {
            printf("\n\tPlease Enter valid children no.(0 or above 0)\n");
            while((ch=getchar())!='\n' && ch!=EOF);   //preventing user from inputting no of children as string.
            continue;
        }
        if(*noc>=0)
            break;
...
FILE *fp;   //file ptr from which input data is taken
...
// fn to print tabs which is useful in making indented display of tree.
void printTab()
{
    printf("\t");
}
...
{   int i,j;
    ...
    if(root==NULL)
    {
        printf("\n\tFamily tree is empty ....please create one\n");
        return;
    }
    printf("%c %s\n",254,(char*)(root->data));
    temp=root->arr;
       ...
       if(scanf("%d",&opt)!=1)
        ...
        switch(opt)
        {
            ...
            case 3:
                if(root==NULL)
                {
                    printf("\n\tFamily Tree is Empty.... Please Create it to search\n");
                    continue;
                }
                display(root,&n);
            ...
            case 4:
                ...
                if(root==NULL)
                {
                    printf("\n\tFamily tree is empty...  Please create it to export");
                    continue;
                }
                root=exportToFile(root,fexport);
                printf("\n\tData Successfuly exported to file....\n");
                fclose(fexport);
				break;
            ...
            case 5:
                if(root==NULL)
                {
                    printf("\n\tFamily Tree is already empty... create it to Destroy.");
                    continue;
                }
